...
│
├── [1~72]      Poki SDK 연동
├── [73~130]    캔버스 초기화 & 반응형 크기 (v2.1: 가로모드 최적화)
├── [131~175]   게임 상태 변수들 (v2.1: chainLightnings, showRotateOverlay)
├── [176~395]   SoundManager (v2.1: +5종 arrowCrit/lightningChain/bossShield/bossSummon/bossSpeedBurst)
├── [396~500]   맵 & 경로 시스템 + 앰비언트 시스템
├── [501~600]   타워 종류 정의 (v2.1: 화살탑 critChance/critMult, 번개탑 chain/chainRange/chainDecay)
├── [600~720]   타워/적/투사체 클래스 (v2.1: Enemy +shield/summon/speedBurst, Projectile +isCrit)
├── [720~820]   웨이브 & 난이도 시스템 (v2.1: HP/속도/수량 상향, 빠른적 방어력)
├── [820~920]   applyDamageToEnemy 헬퍼 + update() 시작
├── [920~1300]  ★ update() - 핵심 게임 로직 (v2.1: 보스능력, 치명타, 체인번개, 가로모드)
├── [1300~2200] ★ draw() - 화면 렌더링 (v2.1: 체인번개 시각효과, 회전 오버레이)
├── [2200~2750] 타워/적 그리기 (v2.1: 보스 방어막/가속 시각효과, 실드바)
├── [2750~2960] 입력 처리
├── [2960~3010] 게임 리셋 (v2.1: chainLightnings 초기화)
└── [3010~3070] 게임 루프 (v2.1: showRotateOverlay 시 일시정지)
```
...
```js
baseHp = 30 + wave × 18 + wave^1.7 × 6     // 더 가파른 지수 증가
count  = 5 + wave × 1.8 + floor(wave/8) × 3  // 수량 증가 + 8웨이브마다 추가
speed  = 1.0 + wave × 0.04 + floor(wave/10) × 0.05  // 빠른 속도 상승
gold   = 5 + wave × 0.6                      // 보상 보정
```
...
| 1      | ~54  | 6     | 일반 6                                |
| 3      | ~110 | 10    | 일반 10 + 빠른 1                      |
| 5      | ~189 | 14    | 일반 14 + 빠른 3 + 탱크 1 + **보스**  |
| 10     | ~390 | 26    | 일반 23 + 빠른 6 + 탱크 3 + **보스**  |
| 20     | ~974 | 45    | 일반 41 + 빠른 12 + 탱크 7 + **보스** |
...
### 가로 모드 최적화
- 여백 축소: 가로 16→4px, 세로 60→8px
- MIN_TILE: 작은 화면에서 36→28로 하향 (레티나 해상도라 가독성 유지)
...
### 상단 바 버튼 고정 너비
- 속도(`TILE×1.5`), 볼륨(`TILE×0.75`), 언어(`TILE×0.9`) — 텍스트/언어 전환 시 크기 불변
- `[Q]` 힌트: 속도 버튼 아래 별도 작은 텍스트 (데스크톱만)
- `btnGap = max(6, TILE×0.15)` 균일 간격

### 스탯 뱃지 겹침 해결
- 3개 버튼 총 너비(`speed+vol+lang+gap×4`) 미리 계산하여 뱃지 가용 영역 예약
- 기존 `speedReserved` → `rightBtnsW` 전체 너비로 교체

### 시작 프롬프트 중앙 정렬
- `measureText()` 기반 동적 박스 너비, 양쪽 패딩 `TILE×0.5`, 최소 `TILE×4`
- `boxX = W/2 - boxW/2`로 항상 중앙

### 게임오버 재시작 버튼
- 깜빡이는 텍스트 → 파란 테두리 둥근 사각형 버튼
- 광고 부활(초록) 위 + 재시작(파란) 아래 레이아웃
- 클릭 핸들러: 버튼 영역만 감지, `pokiCommercialBreak()` 제거 → `restartGame()` 직접 호출
- `window._restartBtn` 히트박스 저장
- 로컬라이제이션: `restartBtn` 키 추가 ('다시 시작' / 'Restart')
