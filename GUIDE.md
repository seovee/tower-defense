...
├── [4600~4930] ★ drawEnemyShape — 친숙 RPG 모티프 5종 (좀비/쥐/늑대/거인/드래곤, v3.4.0 Phase 1)
...
### 적 시각 디자인 (`drawEnemyShape(cx, cy, s, type, flash)`)

**v3.4.0 Phase 1**에서 세균/바이러스 테마 → 친숙 RPG 모티프로 전면 교체. 적 type 코드명·HP 공식·스폰 로직·기믹은 그대로 유지(시각만 교체).

| 타입 | 모티프 | 핵심 시각 단서 |
|------|--------|---------------|
| normal | 🟢 슬라임 | 젤리 방울 몸(quadraticCurve 4개) + **균등 scale 펄스(0.95~1.10, 객체별 위상)** — Enemy.animPhase로 비동기 + 라디얼 그라디언트 입체감(`#a8e878 → #4ea838 → #235818`) + 광택 하이라이트 타원 + 큰 흰자 눈 + 검정 동공 + 동공 하이라이트 + 헤벌쭉 미소 호. 모든 좌표가 펄스된 ss 사용해 균등 변형 |
| swarm | 🐀 쥐 (강화) | 분리된 주둥이(앞쪽 튀어나온 밝은 타원) + 그라디언트 몸통(흰자 톤) + 옅은 배색 + 분홍 귀 외곽/내부 분리 + 흰자+동공+하이라이트 3겹 눈 + 분홍 코 + 입 갈라짐 선 + 정돈된 수염 4개 + 분홍 발 4개 + 두께 변화 꼬리(끝 얇아짐) |
| fast | 🐺 늑대 | 회색 가로 몸통 + 앞쪽 머리 + 뾰족한 귀 2개 + 빨간 눈 + 검정 코 + 흰 송곳니 2개 + 흔들리는 꼬리 + 달리는 4족(sin 위상 흔들림) |
| tank | 🗿 모아이 석상 | 단순화: 둥근 모서리 직사각 머리(quadraticCurve 4코너) + 깊은 눈 2개(검정 가로 사각형 그늘) + 큰 코(사다리꼴) + 굳은 입(가로 직선) — 사용자 피드백 "레이아웃 이상·단순하게" 반영해 헬멧/어깨/보석/턱/돌질감 등 잔디테일 제거 |
| boss | 🐲 드래곤 | 빨간 몸통(박동) + 뿔 2개(노란 톤) + 빛나는 노랑/빨강 눈 + 흰 송곳니 4개 + 등 가시 5개(중앙 가장 큼) + 펄럭이는 양 날개(어두운 빨강) + 비늘 V 패턴 |

**v3.4.0 Phase 2 신규 적 4종** (`spider`/`bear`/`bomber`/`healer` — 시각 + 게임플레이 로직):

| 타입 | 모티프 | 시각 | 기믹 | 등장 |
|------|--------|------|------|------|
| spider | 🕷️ 거미 | 검정 몸 + 8개 다리(살짝 흔들림) + 빨간 모래시계 등 표식 + 빨간 눈 4개 + 송곳니 | `webCooldown` 2±1초마다 가까운 다른 적(2.5타일 내)에 `slowImmuneTimer = 4초` 부여, 🕸 floating | 웨이브 6+ |
| bear | 🐻 곰 | 갈색 둥근 몸(그라디언트) + 둥근 귀(분홍 갈색 안쪽) + 주둥이 분리(밝은 톤) + 작은 눈 + 4족 발 + 코·입 V | HP 1.4× / speed 0.85× / armor 1 | 웨이브 5+ |
| bomber | 💣 자폭병 | 작은 고블린 + 등에 검정 폭탄 + 도화선 흔들림 + 끝 불꽃 펄스 + 광기 노랑 눈 + 뾰족 이빨 + 멜빵 | 처치 시 큰 폭발 파티클 30개 + 충격파 + **lives -1** + screenShake | 웨이브 7+ |
| healer | 🧙 사제 | 보라 로브(그라디언트 사다리꼴) + 어두운 후드 + 황금 십자 + 보라 빛 눈 + **녹색 회복 오브**(객체별 펄스) + 글로우 | `healCooldown` 0.6±0.2초마다 1타일 내 적 HP +2.5%, ✚ floating | 웨이브 8+ |

`Enemy.slowImmuneTimer`가 양수면 매 프레임 `slowTimer/slowAmount`를 0으로 reset하여 슬로우 무력화. spawnDeathParticles 색상 매핑도 type별 추가.

모두 `flash=true` 시 흰색 실루엣(`createWhiteSilhouette` 프리렌더 대체)으로 피격 플래시 표현. 시각 디테일은 `flash` 모드에서 생략(if (flash) return; 패턴).

**객체별 애니메이션 위상**: `Enemy.animPhase = Math.random() × 2π`로 객체마다 비동기. drawEnemyShape(cx, cy, s, type, flash, **phase**) 시그니처. 슬라임 펄스(0.95~1.10) / 사제 오브 펄스 등에 사용.
